...
export default function TestPlayer() {
    ...
    const syncResponseToBackend = async (questionId: number, data: any) => {
        if (!attemptId || isNaN(Number(attemptId)) || String(attemptId) === 'undefined') {
            console.warn('[SYNC] Skipping sync: attemptId is invalid', attemptId);
            return;
        }
        try {
            await api.post(`/attempts/${attemptId}/responses`, {
                ...
                time_taken_sec: 0 // Track actual time later
            });
        } catch (e) {
    ...
    const handleSubmit = async () => {
        console.log('[SUBMIT] handleSubmit called. Attempt ID:', attemptId, 'isSubmitting:', isSubmitting);

        if (!attemptId || isNaN(Number(attemptId)) || String(attemptId) === 'undefined') {
            toast.error('No active mission session. Please refresh and try again.');
            console.error('[SUBMIT] invalid attemptId:', attemptId);
            return;
        }

        if (isSubmitting) return;

        const confirmSubmit = window.confirm('Are you sure you want to submit this test? This action cannot be undone.');
        if (!confirmSubmit) return;

        setIsSubmitting(true);
        try {
            console.log('[SUBMIT] Dispatching POST request to /attempts/' + attemptId + '/complete');
            const response = await api.post(`/attempts/${attemptId}/complete`);
            console.log('[SUBMIT] Response received:', response.status, response.data);
                ...
                <div className="h-20 bg-card-bg border-b border-card-border px-8 flex items-center justify-between shadow-sm">
                    <div className="flex items-center gap-6">
                        ...
                        <div className="hidden md:block">
                            ...
                            <p className="text-[9px] font-bold text-foreground/30 uppercase tracking-widest mt-0.5">Mission Code: BP-2025-AEDO</p>
                        </div>
                    </div>
                            ...
                            <div className="space-y-8">
                                ...
                                {currentQ?.image_path && (
                                    <div className="rounded-[2.5rem] overflow-hidden border border-card-border shadow-2xl">
                                        <img
                                            src={currentQ.image_path.startsWith('http') ? currentQ.image_path : `http://localhost:8000/storage/${currentQ.image_path}`}
                                            alt="Question Visual"
                                        ...
                                        >
                                            ...
                                            {opt.image_path && (
                                                <div className="rounded-2xl overflow-hidden border border-card-border/50">
                                                    <img
                                                        src={opt.image_path.startsWith('http') ? opt.image_path : `http://localhost:8000/storage/${opt.image_path}`}
                                                        alt="Option Visual"
                    ...
                    <div className={cn(
                        ...
                        <div className="p-8 border-b border-card-border flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[8px] font-black uppercase text-foreground/20 italic">{syncing ? 'Uploading Vector...' : 'Real-time Sync'}</span>
                            </div>
                        </div>
