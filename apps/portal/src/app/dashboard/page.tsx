...
export default function Dashboard() {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            // silent — guest or network error, handled below
        } finally {
            setLoading(false);
        }
    };
    ...
    if (!user) {
        return (
            <div className="max-w-5xl mx-auto px-4 py-16 space-y-16 animate-in fade-in duration-700">
                {/* Hero */}
                <div className="text-center space-y-6">
                    <div className="inline-flex items-center gap-2 bg-blue-600/10 border border-blue-500/20 px-5 py-2 rounded-full">
                        <Zap className="h-3.5 w-3.5 text-blue-500" />
                        <span className="text-[10px] font-black text-blue-500 uppercase tracking-widest">Free Access — No Sign-Up Required</span>
                    </div>
                    <h1 className="text-5xl font-black text-foreground uppercase tracking-tighter italic leading-tight">
                        Welcome to <span className="text-blue-600">OSDHYAN</span>
                    </h1>
                    <p className="text-foreground/50 font-bold text-lg max-w-xl mx-auto">
                        Explore all study materials, test series, and focus tools freely. Create an account only when you want to track your progress.
                    </p>
                    <div className="flex items-center justify-center gap-4 pt-2">
                        <Link
                            href="/auth/signup"
                            className="flex items-center gap-2 px-8 py-4 bg-blue-600 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-blue-700 transition-all shadow-xl shadow-blue-500/20 active:scale-95"
                        >
                            Join Free <ArrowRight className="h-4 w-4" />
                        </Link>
                        <Link
                            href="/auth/login"
                            className="flex items-center gap-2 px-8 py-4 border border-card-border text-foreground/60 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:border-blue-500/40 hover:text-blue-500 transition-all"
                        >
                            Sign In
                        </Link>
                    </div>
                ...
                {/* Quick access tiles */}
                <div>
                    <p className="text-[10px] font-black text-foreground/30 uppercase tracking-widest mb-6 text-center">Browse Without Login</p>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        {[
                            { label: 'Test Series', sub: 'Browse 500+ exam series', href: '/dashboard/test-series', icon: Trophy, color: 'bg-blue-600', glow: 'shadow-blue-500/20' },
                            { label: 'Study Materials', sub: 'Notes, PDFs & NCERT', href: '/dashboard/materials', icon: BookOpen, color: 'bg-purple-600', glow: 'shadow-purple-500/20' },
                            { label: 'Pomodoro Studio', sub: 'Focus timer & task planner', href: '/dashboard/focus', icon: Timer, color: 'bg-green-600', glow: 'shadow-green-500/20' },
                            { label: 'Free Quizzes', sub: 'Practice without account', href: '/dashboard/tests', icon: Sparkles, color: 'bg-orange-500', glow: 'shadow-orange-500/20' },
                            { label: 'Syllabus', sub: 'Full exam roadmaps', href: '/dashboard/syllabus', icon: Target, color: 'bg-red-500', glow: 'shadow-red-500/20' },
                            { label: 'PYQ Papers', sub: 'Previous year questions', href: '/dashboard/pyqs', icon: RotateCcw, color: 'bg-indigo-600', glow: 'shadow-indigo-500/20' },
                        ].map(({ label, sub, href, icon: Icon, color, glow }) => (
                            <Link
                                key={href}
                                href={href}
                                className="group bg-card-bg border border-card-border p-8 rounded-[2.5rem] hover:shadow-xl transition-all hover:border-blue-500/20 flex flex-col gap-4"
                            >
                                <div className={cn("h-12 w-12 rounded-2xl flex items-center justify-center text-white shadow-lg", color, glow)}>
                                    <Icon className="h-6 w-6" />
                                </div>
                                <div>
                                    <p className="font-black text-foreground uppercase tracking-tight group-hover:text-blue-600 transition-colors">{label}</p>
                                    <p className="text-[10px] font-bold text-foreground/40 uppercase tracking-widest mt-0.5">{sub}</p>
                                </div>
                                <ArrowRight className="h-4 w-4 text-foreground/20 group-hover:text-blue-600 group-hover:translate-x-1 transition-all mt-auto" />
                            </Link>
                        ))}
                    </div>
                </div>
                ...
                {/* Sign-up value prop */}
                <div className="bg-blue-600/5 border border-blue-500/20 rounded-[2.5rem] p-10 text-center space-y-4">
                    <p className="text-[10px] font-black text-blue-500 uppercase tracking-widest">Sign up to unlock</p>
                    <h2 className="text-2xl font-black text-foreground uppercase tracking-tight italic">Track Your Progress & Set Goals</h2>
                    <div className="flex flex-wrap justify-center gap-3 pt-2">
                        {['Save test scores', 'Goal setup wizard', 'Study streak tracker', 'AI performance analysis', 'Daily planner', 'Leaderboard rank'].map(f => (
                            <span key={f} className="flex items-center gap-1.5 bg-background border border-card-border px-4 py-2 rounded-full text-[10px] font-black text-foreground/60 uppercase tracking-widest">
                                <CheckCircle2 className="h-3 w-3 text-green-500" /> {f}
                            </span>
                        ))}
                    </div>
                    <Link href="/auth/signup" className="inline-flex items-center gap-2 mt-4 px-10 py-4 bg-blue-600 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-blue-700 transition-all shadow-xl shadow-blue-500/20">
    ...
    // API fetch failed for logged-in user
    if (!plannerData) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-background">
                <p className="text-sm font-black text-foreground/40 uppercase tracking-widest">Failed to load dashboard data</p>
                <button
                    onClick={fetchDashboardData}
                    className="px-6 py-3 bg-blue-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all"
                >
                    Retry
                </button>
            </div>
