...
export default function AnalyticsPage() {
    ...
    const fetchData = async () => {
        try {
            ...
            // data remains null — empty state shown below
        } finally {
            setLoading(false);
        }
    };

    // Use server-provided trend data when available; fall back to an empty array
    // so the chart renders cleanly instead of showing stale hardcoded values.
    const accuracyData: { date: string; score: number }[] = data?.score_trend ?? [];
    ...
    const avgTimeSec = data?.stats?.avg_time_per_question_sec;
    const avgTimeDisplay = avgTimeSec != null ? `${avgTimeSec}s` : '—';
    const syllabusCovered = data?.stats?.syllabus_covered_percentage != null
        ? `${data.stats.syllabus_covered_percentage}%`
        : '—';
        ...
        <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    ...
                    { label: 'Avg. Time/Question', value: avgTimeDisplay, icon: Clock, color: 'text-purple-600', bg: 'bg-purple-50' },
                    { label: 'Syllabus Covered', value: syllabusCovered, icon: Zap, color: 'text-yellow-600', bg: 'bg-yellow-50' },
                ].map((stat, i) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                    <div className="flex items-center justify-between mb-8">
                        ...
                        {data?.stats?.accuracy_trend != null && (
                            <div className={cn(
                                "flex items-center gap-1 text-sm font-bold px-2.5 py-1 rounded-full",
                                data.stats.accuracy_trend >= 0
                                    ? "text-green-600 bg-green-50"
                                    : "text-red-600 bg-red-50"
                            )}>
                                <TrendingUp className="h-4 w-4" />
                                {data.stats.accuracy_trend >= 0 ? '+' : ''}{data.stats.accuracy_trend}%
                            </div>
                        )}
                    </div>
                    ...
                    <div className="space-y-6 relative z-10">
                        {data?.ai_suggestion ? (
                            <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/20">
                                <h3 className="text-blue-100 text-sm font-bold mb-1 uppercase tracking-wider">Insight</h3>
                                <p className="text-white font-medium">{data.ai_suggestion}</p>
                            </div>
                        ) : (
                            <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/20 text-center">
                                <p className="text-blue-100 text-sm font-medium">Complete more tests to unlock AI-powered recommendations.</p>
                            </div>
                        )}
                        {data?.ai_time_suggestion && (
                            <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/20">
                                <h3 className="text-blue-100 text-sm font-bold mb-1 uppercase tracking-wider">Time Strategy</h3>
                                <p className="text-white font-medium">{data.ai_time_suggestion}</p>
                            </div>
                        )}
